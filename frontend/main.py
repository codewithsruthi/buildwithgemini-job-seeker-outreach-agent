...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            ...
            # Check if root.data contains wrapped {"kind":"data", "data": ..., "metadata": ...}
            if isinstance(data_val, dict) and "kind" in data_val and "data" in data_val:
                ...
                if inner_mime == _A2UI_MIME:
                    out.append({"kind": "a2ui", "data": data_val.get("data")})
                    continue
