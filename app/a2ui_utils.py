...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        # Extract text preview from components for display fallback
        text_lines = []
        for m in messages:
            su = m.get("surfaceUpdate", {})
            for comp in su.get("components", []):
                txt_obj = comp.get("component", {}).get("Text", {}).get("text", {})
                if "literalString" in txt_obj:
                    text_lines.append(txt_obj["literalString"])
        text_summary = "\n".join(text_lines) if text_lines else "Drafted message created."

        new_parts = [types.Part(text=text_summary)] + [_wrap_a2ui_part(m) for m in messages]
        return LlmResponse(
